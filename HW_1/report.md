...
![Scratch curves](output.png) 
![Pretrained curves](outputtransver.png)
...
The pretrained model reaches 95.87% test accuracy compared to only 34.60% for the scratch model. This makes sense because transfer learning from ImageNet gives the model a head start. It already knows how to detect edges, textures, and shapes, so it converges quickly and to a much better solution. The scratch model struggles because the dataset is small (75 images per class for training), and learning useful features from scratch with that little data is really difficult for a deep network like EfficientNet-B0.
